...
/**
 * KB review-due digest — lists KB entries whose scheduled review date has
 * passed (or whose last verification is stale). One email covers all overdue
 * entries so admins get a single actionable list, not a flood.
 *
 * `data.entries` is a newline-separated list of "JURISDICTION | detail" rows,
 * assembled by the caller.
 */
function kbReviewDueTemplate(data: TemplateData): {
  html: string;
  text: string;
  subject: string;
} {
  const count = escapeHtml(data['count'] ?? '0');
  const dashboardUrl = escapeUrl(data['dashboard_url'] ?? '#');
  const rows = (data['entries'] ?? '').split('\n').filter(Boolean);

  const rowsHtml = rows.length > 0
    ? rows.map((r) => `<li style="margin-bottom: 6px;">${escapeHtml(r)}</li>`).join('')
    : '<li>No entries listed.</li>';

  const html = wrapInLayout(
    `<div class="header">
      <h1>KB entries due for review</h1>
    </div>
    <div class="content">
      <div style="background: #fef3c7; border-left: 4px solid #d97706; padding: 12px 16px; margin-bottom: 16px;">
        <p style="margin: 0; font-weight: 600; color: #92400e;">${count} knowledge-base entr${count === '1' ? 'y is' : 'ies are'} past the scheduled review date.</p>
      </div>
      <p>These entries have passed their <code>next_review_due</code> date (or their last verification is stale). Re-verify each against the primary sources and update <code>last_verified</code> / <code>next_review_due</code>.</p>
      <ul style="font-size: 14px;">${rowsHtml}</ul>
      <p><a class="cta" href="${dashboardUrl}">Open Admin Dashboard</a></p>
      <div style="background: #eff6ff; border: 1px solid #3b82f6; padding: 12px 16px; margin-top: 16px; border-radius: 4px;">
        <p style="margin: 0; font-size: 13px; color: #1e40af;">This is a scheduled review reminder, not a detected change. It fires when an entry is overdue for its periodic human review.</p>
      </div>
    </div>`,
    `${count} KB entries due for review`,
  );

  const rawCount = data['count'] ?? '0';
  const text = `KB Entries Due for Review

${rawCount} knowledge-base entries have passed their scheduled review date (or last verification is stale).

${rows.length > 0 ? rows.map((r) => `- ${r}`).join('\n') : '- No entries listed.'}

Re-verify each against its primary sources and update last_verified / next_review_due.

Admin Dashboard: ${data['dashboard_url'] ?? 'Log in to your admin account'}

---
This is a scheduled review reminder from the Resolvaio Law Monitor system.`;

  return {
    html,
    text,
    subject: `[Review Due] ${rawCount} KB entr${rawCount === '1' ? 'y' : 'ies'} past review date`,
  };
}
...
export function renderTemplate(
  ...
  switch (templateId) {
    ...
    case 'kb_review_due':
      return kbReviewDueTemplate(data);
    default: {
